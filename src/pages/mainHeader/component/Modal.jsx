...
//   if (!open) return null;
  ...
  return (
    <div
      onClick={onClose}
      className={`z-50 fixed inset-0 bg-black/30 flex items-center justify-center transition-opacity duration-300 ${
        open ? "opacity-100 visible" : "opacity-0 invisible"
      }`}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className={`bg-manual-dark rounded-lg w-1/2 overflow-hidden border border-gray-700 transform transition-all duration-300 ease-in-out
      ${
        open
          ? "scale-100 translate-y-0 opacity-100"
          : "scale-95 translate-y-20 opacity-0"
      }`}
      >
        {!movieDetails ? (
          <p>Loading...</p>
              ...
              <ul className="flex items-center gap-3 my-3">
                {movieDetails.genres &&
                  movieDetails.genres.slice(0, 2).map((genre) => (
                    <li
                      key={genre.id}
                      className="text-white/60 font-lato text-sm py-1 px-3 bg-gray-700 rounded"
                    >
                      {genre.name}
                    </li>
                  ))}
                <li className="text-white/60 font-lato text-sm py-1 px-3 bg-gray-700 rounded">
